...
# move ########################################################################
        # if snake go to opposite direction???
        if direction == UP:
...
# network update ##############################################################
        if len(buffer) > BATCH_SIZE:
            ...
            finish      = np.array  ([i[4] for i in minibatch])  # false/true (or 0/1)
...
def currentMatrix(wormCoords, apple):
    # 0: empty / 1: head / 2: apple / 3: else(obstacle)
    ...
    # head_index = wormCoords[HEAD]['x']+1 + (wormCoords[HEAD]['y']+1)*CELLHEIGHT
